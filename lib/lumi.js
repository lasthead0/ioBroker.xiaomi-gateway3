    ...
    static #DEVICES = [
        ...
        },
        {
            ...
            'spec': [
                ...
                // ['14.2.85', undefined, 'motor', 'cover'], TODO: {0: "close", 1: "open", 2: "stop"}
                // ['14.3.85', 'cfg_param', 'cfg_param'], // TODO:
                // ['14.4.85', 'run_state', 'run_state'], // TODO: {0: "closing", 1: "opening", 2: "stop"}
                [undefined, undefined, DebugOutput, [], [DebugOutputParser]],
                [undefined, undefined, MessagesStat, [], [MessagesStatParser]],
            ]
        },
    ...
    static getDevice(model, extDevices) {
        ...
        if (device != undefined) {
            const deviceSpec = (device['spec'] || []).map(spec => {
                ...
                const ZigbeeStateClass = class extends stateClass {
                    /* Override */
                    normalizeLeft(val) {
                ...
                return [
                    lumi,
                    prop,
                    new ZigbeeStateClass(lumi, ...stateArgs)
                ];
            });
...
/* Maps functions */
function ZeroOneTrueFalseMap() {
    return (val, toState) => {
        const [from, to] = (map => toState ? map : map.reverse)([[0, 1], [true, false]]);

        const returnValue = to[from.indexOf(val)];
        return returnValue != undefined ? returnValue : val;
    };
}

function ZeroOneFalseTrueMap() {
    return (val, toState) => {
        const [from, to] = (map => toState ? map : map.reverse)([[0, 1], [false, true]]);
