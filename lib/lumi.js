 ...
 * [
 *     'state_name',
 *     {name, role, type, read, write, unit, min, max, states},
 *     {valueMap, dependsOn}
 * ]
 */
    ...
    static #DEVICES = [
        ...
        },
        {
            ...
            'spec': [
                [undefined, 'alive', Available, [], [AvailableParser]],
                ['8.0.2007', 'lqi', LinkQuality, [], [DefaultParser]],
                ['0.1.85', undefined, BedActivity, [], [DefaultParser]],
                ['0.2.85', undefined, TiltAngle, [], [DefaultParser]],
                // ['0.3.85', undefined, 'vibrate_intensity'], // TODO:??
                ['13.1.85', undefined, Trigger, ['vibration', {name: 'Vibration'}, {dependsOn: 'action_timeout'}], [VibrationParser]],
                ['13.1.85', undefined, Trigger, ['tilt', {name: 'Tilt'}, {dependsOn: 'action_timeout'}], [TiltParser]],
                ['13.1.85', undefined, Trigger, ['drop', {name: 'Drop'}, {dependsOn: 'action_timeout'}], [DropParser]],
                [undefined, undefined, Timeout, ['action_timeout', {name: 'Action timeout'}]],
                ['13.1.85', undefined, Action, ['last_action', {name: 'Last action', states: {1: 'vibration', 2: 'tilt', 3: 'drop'}}], [DefaultParser]],
                ['14.1.85', undefined, VibrationSensitivity, [], [DefaultParser]],
                ['8.0.2008', 'voltage', Voltage, [], [VoltageParser]],
                ['8.0.2008', 'voltage', Battery, [], [BatteryParser]],
                [undefined, undefined, DebugOutput, [], [DebugOutputParser]],
                [undefined, undefined, MessagesStat, [], [MessagesStatParser]],
        ...
        if (device != undefined) {
            const deviceSpec = (device['spec'] || []).map(spec => {
                ...
                const LumiStateClass = class extends stateClass {
                    /* Override */
                    normalizeLeft(val) {
                        const parsersChainFunc = funcs => val => funcs
                            .reduce((p, pf) => {
                                if (p != undefined) return pf(p, true);
                            }, val);
                        const _val = parsersChainFunc(parsersFuncs)(val);

                        if (_val != undefined)
                            return super.normalizeLeft(_val);
                    }
...
function LoadAvgParser() {
    return val => {
        ...
        if (load_avg != undefined) {
            const [a, b, c] = load_avg.split('|');
            return [
                Number(a).toFixed(2),
                Number(b).toFixed(2),
                Number(c).toFixed(2)
...
function RunTimeParser() {
    return val => {
        ...
        if (run_time != undefined)
            return Math.floor(Number(run_time) / 60);
    };
}
...
function TiltParser(model) {
    return val => (val == 2) ? 1 : 0;
}

function VibrationParser(model) {
    return val => (val == 1) ? 1 : 0;
}

function VoltageParser(model) {
    return val => Number(parseFloat(val / 1000).toFixed(3));
}
