...
class TelnetShell {
    ...
    async readFile(file, base64 = false) {
        return await this._shellExec(async (shell, ...args) => {
            ...
            /* FIXME: Crutch to calc _echoLines_ and get "clean" response */
            /* https://github.com/mkozjak/node-telnet-client/pull/200 */
            const cmd = `cat ${file}${base64 ? ' | base64' : ''}`;
            const recv = await shell.exec(cmd, {echoLines: Math.ceil(cmd.length / 80)});
