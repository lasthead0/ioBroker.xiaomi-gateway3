 ...
 * 
 *     function setter(did, callback, context)
...
class StateClass {
    stateName = undefined;
    stateCommon = {/* name, role, type, read, write, unit, min, max */};
    ...
    _update = (stateName, stateCommon, options) => {
        const isCommonLike = obj => {
            const props = ['name', 'role', 'type', 'read', 'write', 'unit', 'min', 'max'];
        ...
        if (isObject(stateName) && isCommonLike(stateCommon))
            stateCommon = stateName;
        else if (isObject(stateName) && !isCommonLike(stateCommon) && stateCommon == undefined)
            options = stateName;
    ...
    get stateObject() {
        /* */
        const {name, role = 'state', type, read, write, unit, min, max} = this.stateCommon;
