...
module.exports = class Zigbee {
    ...
    static fixXiaomiProp(model, param, val) {
        if (['temperature', 'humidity', 'pressure'].includes(param)) {
            ...
            // # xiaomi cube
            return (val / 1000.0);
        } else if (['consumption', 'power'].includes(param)) {
            if (typeof val === 'number')
                return val.toFixed(2);
            else
                return parseFloat(val).toFixed(2);
        } else if (['on', 'open'].includes(val)) {
            return 1;
        } else if (['off', 'close'].includes(val)) {
            return 0;
        } else if (param == 'voltage') {
            return Number(Number(val / 1000).toFixed(3));
        /* New battery % formula */
        } else if (param == 'battery' && val != undefined) {
            if (val <= 100)
                return val;
            else if (val <= 2700)
                return 0;
