    ...
    */
    ...
    static parseXiaomiBle(msg) {
        ...
        //     return {'sleep': data[0]}  # 1 => true
            ...
            return {
                'temperature': (Number(Buffer.from(data).readInt16LE(0)) / 10.0)
            };
        ...
        //     return {'power': data[0], 'temperature': data[1]}
            ...
            let value = (Number(Buffer.from(data).readInt16LE(0)) / 10.0);
        ...
        //     return {'conductivity': int.from_bytes(data, 'little')}
            ...
            return {
                'temperature': (Number(Buffer.from(data.slice(0, 2)).readInt16LE(0)) / 10.0),
                'humidity': (Number(Buffer.from(data.slice(-2)).readInt16LE(0)) / 10.0)
            };
