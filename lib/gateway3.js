...
class XiaomiDevice {
    ...
    constructor({did, mac, type, model, name, fwVer, spec, stat}) {
        this.#did = did || '';
        this.#mac = mac || '';
        this.#type = type || '';
        this.#model = model || '';
    ...
    set fwVer(v) {this.#fwVer = v}
    get fwVer() {return this.#fwVer}
...
class Gateway3 extends XiaomiDevice {
    ...
    async initialize(config, cb) {
        ...
        if (await this._checkPort(23) != true && await this._enableTelnet(openTelnetCmd) != true) {
            ...
            for (const d of [].concat(lumi1, ble1))
                this.logger.debug(`${d.type.toLowerCase()}: ${d.model}, did: ${d.did}`);
    ...
    async _getDevices() {
        ...
        if (true) {
            try {
                let raw = await this.#shell.readFile('/data/zigbee/device.info');
                const lumiZigbeeDevices = JSON.parse(raw)['devInfo'];

                /*  */
                raw = await this.#shell.readFile(DB_ZIGBEE);
                const lumiDevices1 = JSON.parse(String(raw).replace(/\}[\s\x00]*\{/g, ', '));

                const lumiDevicesProps = lumiZigbeeDevices
                    .map(el => el.did)
                    .reduce((obj, did) => {
                        try {
                            return Object.assign({}, obj, {[did]: JSON.parse(lumiDevices1[`${did}.prop`]).props});
                        } catch (e) {
                            return obj;
                        }
                    }, {});
                ...
                for (let lumi of lumiZigbeeDevices) {
                    ...
                    const retain = lumiDevicesProps[did];

                    if (retain == undefined) {
                        this.logger.error(`${model}, did: ${did} is not in the Xiaomi database`);
                        continue;
                    }
    ...
    processMessageLumi(message, cb, cbd) {
        ...
        const did = message['did'] != 'lumi.0' ? message['did'] : this.did;
    ...
    processMessageLogMiio(message, cb) {
        if (Object.keys(this.devices).includes(this.did) == false)
            return;
        ...
        if (device != undefined && ['lumi', 'zigbee'].includes(type)) {
            ...
            if (isObject(states)) {
                const changedStates = Object.keys(states)
                    ...
                    .filter(([, state]) => state != undefined && state.resource != undefined);

                const payload = changedStates
                    ...
                    .reduce((acc, params) => {
                        ...
                        return Object.assign({'cmd': 'write', 'did': did}, acc, {[key]: [].concat(acc[key] || [], params[key] || [])});
                    }, undefined);
