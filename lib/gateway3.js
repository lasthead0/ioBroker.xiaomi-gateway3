...
class Gateway3 extends XiaomiDevice {
    ...
    async _getDevices() {
        ...
        try {
            ...
            const {name, spec} = Zigbee.getDevice('lumi.gateway.mgl03');
        ...
        if (true) {
            try {
                ...
                for (let lumi of lumiZigbeeDevices) {
                    ...
                    const desc = Zigbee.getDevice(lumiModel);

                    if (desc == undefined) {
                        this.logger.error(`${did} has an unsupported Zigbee model: ${lumiModel}`);
                        continue;
                    }
    ...
    processMessageLumi(message, cb, cbd) {
        ...
        /* << EOF Common part for `processMessageZigbee` and `processMessageBle` */
