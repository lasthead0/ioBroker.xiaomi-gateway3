...
class XiaomiDeviceStat {
    ...
    update(message) {
        if (message['sourceAddress'] != undefined) {
            ...
            this.#rssi = rssi;
            this.#lastMessage = Zigbee.CLUSTERS[clusterId];
            this.#received += 1;
...
class XiaomiDevice {
    ...
    constructor({did, model, mac, type, specLUMI, specMIOT}, resetCnt) {
        this.#did = did || '';
        this.#mac = mac || '';
        this.#type = type || '';
        this.#model = model || '';
        ...
        this.#stat = new XiaomiDeviceStat(resetCnt);
    }
...
class Gateway3 extends XiaomiDevice {
    ...
    async initialize(config, cb) {
        ...
        const {telnetCmd, gwEnablePublicMqtt, gwLockFirmware, gwDisableBuzzer, gwInMemory} = config;
            ...
            try {
                ...
                for (let lumi of lumiZigbeeDevices) {
                    ...
                    devices.push({
                        ...
                        specMIOT,
                        'init': Object.assign({},
                            Zigbee.fixXiaomiProps(zigbeeModel, params),
    ...
    _setupDevices(_devices, findOrCreateDevice) {
        for (let _device of _devices) {
            ...
            this.devices[did] = new XiaomiDevice({did, mac, model, specLUMI, specMIOT}, resetCnt);
            if (type == 'gateway') {
                this.did = did;
    ...
    processMessageZigbee(message, cb, cbd) {
        const {cmd} = message;
        let pkey;
        ...
        const [paramsKeyVal, zigbeeProps] = message[pkey].reduce((pv, cv) => {
            ...
            return [
                Object.assign({}, pv[0] || {}, s != undefined ? {[s]: val} : {}),
                [].concat(pv[1] || [], [prop])
            ];
        }, []);
    ...
    processMessageBle(message, cb, cbd) {
        ...
        const {mac, model, seq: _seq} = this.devices[did];
    ...
    processMessageReceived(message, cb) {
        ...
        if (device != undefined && device.stat != undefined) {
            ...
            const stat = Object.assign({}, device.stat.object,
                {
                    did
                }
            );
