...
class XiaomiDevice {
    ...
    constructor({did, mac, type, model, fwVer, specLumi, specMiot, stat}) {
        this.#did = did || '';
        this.#mac = mac || '';
        this.#type = type || '';
        this.#model = model || '';
...
class Gateway3 extends XiaomiDevice {
            ...
            try {
                ...
                for (let lumi of lumiZigbeeDevices) {
                    ...
                    const params = [].concat(specLumi || [], specMiot || []).reduce((p, [, prop, state]) => {
                        return Object.assign({}, p, {[state]: Zigbee.fixXiaomiProp(model, state, retain[prop])});
                    }, {}) || {};
    ...
    _setupDevices(_devices, findOrCreateDevice) {
        for (let _device of _devices) {
            const {did, mac, type, model, name, fwVer, specLumi, specMiot, init} = _device;
            const {resetCnt} = init || {};
            ...
            if (type == 'gateway') {
                ...
                findOrCreateDevice({
                    mac,
                    model,
                    specs: (specLumi || specMiot || [])
                        .map(el => [el[2], el.length > 3 ? el[3] : undefined])
        ...
        const [paramsKeyVal, zigbeeProps] = message[pkey].reduce((pv, cv) => {
            ...
            const val = (() => {
                if (Object.keys(cv).includes('value')) {
                    return Zigbee.fixXiaomiProp(model, s, cv.value);
